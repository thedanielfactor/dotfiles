...
# Let's define where our memory (timestamp) will live. Think of it as the script's little diary.
timestamp_file="/tmp/.last_ip_check"

# Now, let's see if we've been too eager and checked the time too recently.
if [[ -f "$timestamp_file" ]]; then
    last_run=$(cat "$timestamp_file")
    current_time=$(date +%s)
    time_diff=$((current_time - last_run))

    # If it's been less than a second, let's just chill out and not do anything hasty.
